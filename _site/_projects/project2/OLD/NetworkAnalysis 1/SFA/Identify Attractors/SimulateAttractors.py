...
##  read in full network nodes, and their initial conditions 
    ...
    while n <= num_conditions:
        networkstatesfile=open('test/basal'+ str(n).rjust(len(str(num_conditions)), '0') + '.txt').read()
...
## write out results
        outputfile=open('test2/attractor' + str(n).rjust(len(str(num_conditions)), '0') + '.txt','w+')
        ls = []
