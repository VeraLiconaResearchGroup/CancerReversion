...
class ThreeNodeCascade(sfa.base.Data):
    def __init__(self):
        ...
        fpath = os.path.join(dpath, '../../Network Construction 3/Network_full.sif')
...
##  read in full network nodes, and their initial conditions 
    network=open('nodes_expr_216').read().split('\n')
    networkstatesfile=open('231_basal').read()
...
##  RUN SFA        
    x = alg.compute(b)
## write out results
    outputfile=open('attractor_231.txt','w+')
