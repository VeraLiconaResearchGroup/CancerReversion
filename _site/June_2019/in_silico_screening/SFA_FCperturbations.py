...
##read in readout nodes, readout node attractor state file
    ronfile='readout_nodes_13'
    readout_nodes=open(ronfile).read().split('\n')

    ronssfile='readoutnodes_13_231attrval'
    readout_nodes_state_file=open(ronssfile).read()


##  read in full network nodes, and their initial conditions (basal activity values)
    network=open('network_nodes_173').read().split('\n')
    basal_file='231_basal'
...
##  if i is a readout node, save values to readout_dict, and compare values to logFC
        ...
        if (quantifier/num_readouts)>.616:
            ...
            print(quantifier)
            outputfile=open('SFAoutput_ron_FC1_3.txt','w+') #SFA results
...
##  create output file for FC configuratoin
            FCout=open('SFAoutput_FCconfig_FC1_3.txt','w+')
            for key,value in FCdict.items():
                FCout.write(str(key)+'='+str(value)+'\n')
            fullnetworkout=open('SFAoutput_full_FC1_3.txt','w+')
