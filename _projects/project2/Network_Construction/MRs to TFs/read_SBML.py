...
def get_sign(annotations):
    #for annotation in annotations:
        for signline in annotations:
            if 'Effect' in signline:
                #print(signline)
...
def main():
    reader=SBMLReader()
    document=reader.readSBML('MRs expressed (prtn only) viz all.xml')
...
##Processing Reactions
    ...
    for x in range(0,numreactions):
        ...
        annotations=reaction.getAnnotationString().split('\n')
        #print(annotations)
