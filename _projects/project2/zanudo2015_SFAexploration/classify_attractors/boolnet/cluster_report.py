...
for k in range(2,16):
    ...
    kmeans = pd.read_csv(folder + '/kmeans_' + data  + str(k) + '.txt', delim_whitespace = True, index_col = ['name'])
##    kmeans = pd.read_csv(folder + '/kmeans_attr' + str(k) + '.txt', delim_whitespace = True, index_col = ['name'])

    cl_count = pd.DataFrame(kmeans.clusters.value_counts())
    cl_count.index.name = 'cluster'
    cl_count.columns = ['cluster_size']

    leuk = open('leukemia_boolnet.txt').read().split('\n')
    apop = open('apoptosis_boolnet.txt').read().split('\n')
    ls = [apop,leuk]
    ds = ['apoptosis', 'leukemia']
    clusts = {}
    
    with open('cluster_reports/cluster_report_kmeans_' + data + '_' + str(k) +'.txt', 'w') as f:
