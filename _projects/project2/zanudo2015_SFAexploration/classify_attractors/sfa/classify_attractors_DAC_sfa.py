...
aron = {"apoptosis":1, "bid":1, "ceramide":1, "disc":1, "fas":1, "iap":-1, "flip":-1}
lron = {"apoptosis":0, "s1p":0, "ceramide":0, "disc":0, "fas":0, "flip":0}
...
df2=pd.read_csv('sfa_DAC_disc.txt',delim_whitespace=True,index_col=['name'], dtype = 'float64')
df2 = df2[RONs]
df5 = pd.read_csv('sfa_DAC_disc_vsAPOP.txt',delim_whitespace=True,index_col=['name'], dtype = 'float64')
df5 = df5[RONs]
...
otherdf.to_csv('classify_attractors_newRONs/other_attractors_sfa.txt', sep = " ")
candf.to_csv('classify_attractors_newRONs/leukemia_attractors_sfa.txt', sep = " ")
apopdf.to_csv('classify_attractors_newRONs/apoptosis_attractors_sfa.txt', sep = " ")

with open('classify_attractors_newRONs/leukemia_sfa.txt', 'w') as outfile:
    outfile.write("\n".join(leuk))

with open('classify_attractors_newRONs/apoptosis_sfa.txt', 'w') as outfile:
    outfile.write("\n".join(apop))

with open('classify_attractors_newRONs/other_sfa.txt', 'w') as outfile:
