...
inits = pd.read_csv('L2_initial.txt', delim_whitespace = True, index_col = ["name"])
##states = states.transpose()
fixed_nodes = {"ceramide":0,"pdgfr":1}
##inits = inits.loc[ind,]
inits = inits.loc[[ind]]
...
##    initalize SF Class
    ...
    # Get index of nodes to fix
    ind = {}
    for key in fixed_nodes.keys():
        ind[key] = data.n2i[key]
    print('indices for fixed nodes: ', ind)
    print('orientation for fixed nodes: ', fixed_nodes)


    for index,row in inits.iterrows():
        for node in cols:
##            print(inits.loc[index,node])
##            print(b[data.n2i[node]])
...
##    attr_logss.to_csv('sfa_A6.txt', sep=' ',float_format='%.4f',index_label="name") 
