...
if __name__ == '__main__':
    ...
    pdf = matplotlib.backends.backend_pdf.PdfPages(writedir + "plots.pdf")
    for discretization, data_d in dict_data_discretized.items():
        ...
        disc = discretization.split('/')[-1]
        disc = disc.split('.')[0]
        p = plt.figure()
        x1 = range(len(data_original.flatten()))
        y1 = data_original.flatten()
        y2 = data_d_s.flatten()
        plt.plot(x1, y1, label = "Original")
        plt.plot(x1, y2, label = disc)
        plt.legend()
